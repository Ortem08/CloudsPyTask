...
def check_yandex(directory='all'):
    if directory == 'all':
        files = yandex.get_file_info()
    elif directory == 'root':
        files = yandex.search('/')
    else:
        possible_dirs = yandex.get_file_info(directory, is_folder=True)
        ...
        files = yandex.search(desired_path)
...
def download_yandex(is_dir, name):
    possible_files = yandex.get_file_info(name, is_dir)
    desired_file = None
    ...
    if is_dir:
        if yandex.download_folder(desired_file):
            print("Folder downloaded")
        else:
            print("Smth went wrong")
    else:
        print(yandex.download_file(desired_file))
...
def upload_yandex(is_folder, path):
    if is_folder == 'file':
        yandex.upload_file(path_result='/', path_source=path, replace=True)
    elif is_folder == 'folder':
        yandex.upload_folder(savepath='', loadpath=path)
    else:
        print(f'Wrong parameter {is_folder}')
